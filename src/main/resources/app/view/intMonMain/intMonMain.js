...
(function() {
    ...
    function monDataBuildTable(o) {
        var handlers = {},
            root = o.tag + 's',
            reqF = o.tag + 'DataRequest',
            resp = o.tag + 'DataResponse',
            onSel = fs.isF(o.selCbFF),
            onResp = fs.isF(o.respCbFF),
            idKey = o.idKey || 'id',
            oldTableData = [],
            refreshPromise;
        ...
        o.scope.sortParamsF = o.sortParamsF || {};
        o.scope.autoRefreshF = true;
        o.scope.autoRefreshTipF = 'Toggle auto refresh';

        // === websocket functions --------------------
        // response
        function respCbF(data) {
            ls.stop();
            o.scope.tableDataF = data[root];
            o.scope.annotsF = data.annots;
            onResp && onResp();

            // checks if data changed for row flashing
            if (!angular.equals(o.scope.tableDataF, oldTableData)) {
                o.scope.changedDataF = [];
                // only flash the row if the data already exists
                if (oldTableData.length) {
                    angular.forEach(o.scope.tableDataF, function (item) {
                        if (!fs.containsObj(oldTableData, item)) {
                            o.scope.changedDataF.push(item);
                        }
                    });
                }
                angular.copy(o.scope.tableDataF, oldTableData);
            }
            o.scope.$apply();
        }
        handlers[resp] = respCbF;
        wss.bindHandlers(handlers);

        // request
        function sortCbF(params) {
            var p = angular.extend({}, params, o.queryF);
            if (wss.isConnected()) {
                wss.sendEvent(reqF, p);
                ls.start();
            }
        }
        o.scope.sortCallbackF = sortCbF;


        // === selecting a row functions ----------------
        function selCbF($event, selRow) {
            var selId = selRow[idKey];
            o.scope.selIdF = (o.scope.selIdF === selId) ? null : selId;
            onSel && onSel($event, selRow);
        }
        o.scope.selectCallbackF = selCbF;

        // === autoRefresh functions ------------------
        function fetchDataIfNotWaitingF() {
            if (!ls.waiting()) {
                if (fs.debugOn('widget')) {
                    $log.debug('Refreshing ' + root + ' page');
                }
                sortCbF(o.scope.sortParamsF);
            }
        }

        function startRefreshF() {
            refreshPromise = $interval(fetchDataIfNotWaitingF, refreshInterval);
        }

        function stopRefreshF() {
            if (refreshPromise) {
                $interval.cancel(refreshPromise);
                refreshPromise = null;
            }
        ...
        function toggleRefreshF() {
            o.scope.autoRefreshF = !o.scope.autoRefreshF;
            o.scope.autoRefreshF ? startRefreshF() : stopRefreshF();
        }
        o.scope.toggleRefreshF = toggleRefreshF;
